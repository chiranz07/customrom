...
object ShadeColors {
    ...
    /**
     * True when the shade/QS surfaces should be flat black instead of the Monet fallback tint.
     *
     * With blur off there is no translucency for the tint to sit in, so `shade_panel_fallback`
     * (`system_accent2_800`) reads as a flat Monet-coloured QS header. In dark mode we use black
     * instead, whether or not the "Pure black" theme is on.
     *
     * NOTE: the previous version of this gated on `berry_black_theme` read from AOSP
     * `Settings.Secure` — but that setting lives in the LineageSettings provider (see
     * DarkModeBlackThemeStorage and ThemeOverlayController), so the read always returned 0 and the
     * black branch was dead code. Do not "restore" that read against Settings.Secure.
     */
    @JvmStatic
    private fun useBlackShadeFallback(context: Context): Boolean {
    ...
    @JvmStatic
     private fun shadePanelFallback(context: Context): Int {
        return if (useBlackShadeFallback(context)) {
            Color.BLACK
        } else {
    ...
    private fun notificationScrimFallback(context: Context): Int {
        return if (useBlackShadeFallback(context)) {
            ...
            context.getColor(R.color.notification_scrim_fallback)
        }
    }
}
