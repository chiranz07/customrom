...
while [ "$ATTEMPT" -lt "$MAX_ATTEMPTS" ]; do
    ATTEMPT=$((ATTEMPT + 1))
    LOGFILE=~/mistb_husky_attempt${ATTEMPT}.log
    ...
    bash -c "source build/envsetup.sh > ~/envsetup_husky_attempt${ATTEMPT}.log 2>&1 && lunch mist_husky-aosp_current-userdebug > ~/lunch_husky_attempt${ATTEMPT}.log 2>&1 && mist b -j${JOBS}" > "$LOGFILE" 2>&1
    RC=$?
    echo "=== [$(date)] Attempt $ATTEMPT exited rc=$RC ===" >> "$LOGFILE"

    if [ "$RC" -eq 0 ]; then
        echo "BUILD_SUCCEEDED attempt=$ATTEMPT" > ~/mist_build_final_status_husky.txt
        exit 0
    fi
    ...
    if grep -qE "ninja failed with: signal: (killed|terminated)" "$LOGFILE"; then
        ...
        echo "REAL_BUILD_FAILURE attempt=$ATTEMPT logfile=$LOGFILE" > ~/mist_build_final_status_husky.txt
        exit 2
    else
...
echo "MAX_ATTEMPTS_EXCEEDED" > ~/mist_build_final_status_husky.txt
exit 3
